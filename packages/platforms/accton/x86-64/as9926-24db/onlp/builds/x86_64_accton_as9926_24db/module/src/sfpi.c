...
int onlp_sfpi_presence_bitmap_get(onlp_sfp_bitmap_t* dst)
{
	...
	if(fp == NULL) {
		syslog(LOG_ERR, "Unable to open the module_present_all \
			      device file from (%s).", 
	...
	if(count != 4) {
		...
		syslog(LOG_ERR, "Unable to read all fields the \
...
int onlp_sfpi_rx_los_bitmap_get(onlp_sfp_bitmap_t* dst)
{
	...
	if(fp == NULL) {
		syslog(LOG_ERR, "Unable to open the module_rxlos_all \
			      device file from (%s).", 
	...
	if(count != 1) {
		/* Likely a CPLD read timeout. */
		syslog(LOG_ERR, "Unable to read all fields the module_rxlos_all \
...
int onlp_sfpi_eeprom_read(int port, uint8_t data[256])
{
	...
	if (size != 256) {
		syslog(LOG_ERR, "Unable to read eeprom from port(%d), size is \
			      different!", port);
		return ONLP_STATUS_E_INTERNAL;
	}
...
int onlp_sfpi_dom_read(int port, uint8_t data[256])
{
	...
	fp = fopen(file, "r");
	if(fp == NULL) {
		syslog(LOG_ERR, "Unable to open the eeprom device file of \
			      port(%d)", port);
		return ONLP_STATUS_E_INTERNAL;
	}
	...
	if (fseek(fp, 256, SEEK_CUR) != 0) {
		fclose(fp);
		syslog(LOG_ERR, "Unable to set the file position indicator of \
			      port(%d)", port);
		return ONLP_STATUS_E_INTERNAL;
	}
	...
	fclose(fp);
	if (ret != 256) {
		syslog(LOG_ERR, "Unable to read the module_eeprom device file of \
			      port(%d)", port);
		return ONLP_STATUS_E_INTERNAL;
	}
...
int onlp_sfpi_control_set(int port, onlp_sfp_control_t control, int value)
{
	...
	case ONLP_SFP_CONTROL_TX_DISABLE_CHANNEL:
	{
		...
		if (present == 1) {
			if (port >= SFP_PORT_MIN && port <= SFP_PORT_MAX) { /* SFP */
				...
				if (identifier < 0) {
					syslog(LOG_ERR, "Unable to write tx_disable status to port(%d): read identifier from eeprom fail", port);
					rv = ONLP_STATUS_E_INTERNAL;
				}
				else if (identifier == QSFP_DD_IDENTIFIER) { /* QSFP DD */
					/* Flat-memory CMIS modules do not implement page 01h/10h */
					status_byte = onlp_sfpi_dev_readb(port, PORT_EEPROM_DEVADDR, QSFP_DD_LOWER_OFFSET_STATUS);
					if (status_byte < 0) {
						syslog(LOG_ERR, "Unable to write tx_disable status to port(%d): read status byte from eeprom fail", port);
						rv = ONLP_STATUS_E_INTERNAL;
					}
						...
						if (eeprom_control < 0) {
							...
							else if (onlp_sfpi_dev_writeb(port, PORT_EEPROM_DEVADDR, QSFP_DD_EEPROM_P10H_OFFSET_OUTPUT_DISABLE_TX, value) < 0) {
	...
	case ONLP_SFP_CONTROL_LP_MODE:
	{
		...
		if (present == 1) {
			identifier = onlp_sfpi_dev_readb(port, PORT_EEPROM_DEVADDR, QSFP_EEPROM_OFFSET_IDENTIFIER);
			if (identifier < 0) {
				syslog(LOG_ERR, "Unable to write LP mode status to port(%d): read identifier from eeprom fail", port);
				rv = ONLP_STATUS_E_INTERNAL;
			}
			else if (identifier == QSFP_DD_IDENTIFIER) { /* QSFP DD */
				lpmode_value = onlp_sfpi_dev_readb(port, PORT_EEPROM_DEVADDR, QSFP_DD_EEPROM_OFFSET_LOWPWR_REQUEST_SW);
				if (lpmode_value < 0) {
					syslog(LOG_ERR, "Unable to write LP mode status to port(%d): read LP mode value from eeprom fail", port);
					rv = ONLP_STATUS_E_INTERNAL;
				}
				else {
					if (value)
					...
					if (onlp_sfpi_dev_writeb(port, PORT_EEPROM_DEVADDR, QSFP_DD_EEPROM_OFFSET_LOWPWR_REQUEST_SW, lpmode_value) < 0) {
						syslog(LOG_ERR, "Unable to write LP mode status to port(%d): write LP mode value to eeprom fail", port);
						rv = ONLP_STATUS_E_INTERNAL;
					}
					else {
						rv = ONLP_STATUS_OK;
...
int onlp_sfpi_control_get(int port, onlp_sfp_control_t control, int* value)
{
	...
	case ONLP_SFP_CONTROL_TX_DISABLE_CHANNEL:
	{
		...
		if (present == 1) {
			if (port >= SFP_PORT_MIN && port <= SFP_PORT_MAX) { /* SFP */
				...
				identifier = onlp_sfpi_dev_readb(port, PORT_EEPROM_DEVADDR, QSFP_EEPROM_OFFSET_IDENTIFIER);
				if (identifier < 0) {
					syslog(LOG_ERR, "Unable to read tx_disabled status from port(%d): read identifier from eeprom fail", port);
					rv = ONLP_STATUS_E_INTERNAL;
				}
				else if (identifier == QSFP_DD_IDENTIFIER) { /* QSFP DD */
					/* Flat-memory CMIS modules do not implement page 01h/10h */
					status_byte = onlp_sfpi_dev_readb(port, PORT_EEPROM_DEVADDR, QSFP_DD_LOWER_OFFSET_STATUS);
					if (status_byte < 0) {
						syslog(LOG_ERR, "Unable to read tx_disable status from port(%d): read status byte from eeprom fail", port);
						rv = ONLP_STATUS_E_INTERNAL;
					}
					else if (status_byte & QSFP_DD_FLAT_MEM) {
						rv = ONLP_STATUS_E_UNSUPPORTED;
						...
						syslog(LOG_ERR, "Unable to read tx_disable status from port(%d): write page to eeprom fail", port);
						rv = ONLP_STATUS_E_INTERNAL;
					}
					else {
						support_ctrls = onlp_sfpi_dev_readb(port, PORT_EEPROM_DEVADDR, QSFP_DD_EEPROM_P01H_OFFSET_CONTROL_1);
						if (support_ctrls < 0) {
							syslog(LOG_ERR, "Unable to read tx_disable status from port(%d): read support control from eeprom fail", port);
							rv = ONLP_STATUS_E_INTERNAL;
						}
						else if (support_ctrls & QSFP_DD_P01H_TX_DISABLE_SUPPORT) {
							if (onlp_sfpi_dev_writeb(port, PORT_EEPROM_DEVADDR, QSFP_DD_EEPROM_OFFSET_BANK_SELECT, 0) < 0) {
								syslog(LOG_ERR, "Unable to read tx_disable status from port(%d): write bank to eeprom fail", port);
								rv = ONLP_STATUS_E_INTERNAL;
							}
							else if (onlp_sfpi_dev_writeb(port, PORT_EEPROM_DEVADDR, QSFP_DD_EEPROM_OFFSET_PAGE_SELECT, QSFP_DD_PAGE_LANE_CTRL) < 0) {
								syslog(LOG_ERR, "Unable to read tx_disable status from port(%d): write page to eeprom fail", port);
								...
								tx_dis = onlp_sfpi_dev_readb(port, PORT_EEPROM_DEVADDR, QSFP_DD_EEPROM_P10H_OFFSET_OUTPUT_DISABLE_TX);
								if (tx_dis < 0) {
									syslog(LOG_ERR, "Unable to read tx_disable status from port(%d): read TX disable from eeprom fail", port);
									rv = ONLP_STATUS_E_INTERNAL;
								}
								else {
									*value = tx_dis;
	...
	case ONLP_SFP_CONTROL_LP_MODE:
	{
		...
		if (present == 1) {
			identifier = onlp_sfpi_dev_readb(port, PORT_EEPROM_DEVADDR, QSFP_EEPROM_OFFSET_IDENTIFIER);
			if (identifier < 0) {
				syslog(LOG_ERR, "Unable to read LP mode status from port(%d): read identifier from eeprom fail", port);
				rv = ONLP_STATUS_E_INTERNAL;
			}
			else if (identifier == QSFP_DD_IDENTIFIER) { /* QSFP DD */
				/* lpmode valid bit(bit4): Low power request sw */
				lpmode_value = onlp_sfpi_dev_readb(port, PORT_EEPROM_DEVADDR, QSFP_DD_EEPROM_OFFSET_LOWPWR_REQUEST_SW);
				if (lpmode_value < 0) {
					syslog(LOG_ERR, "Unable to read LP mode status from port(%d): read LP mode value from eeprom fail", port);
					rv = ONLP_STATUS_E_INTERNAL;
				}
				else {

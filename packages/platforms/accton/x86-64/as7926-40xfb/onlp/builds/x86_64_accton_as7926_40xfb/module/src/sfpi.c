...
onlp_sfpi_eeprom_read(int port, uint8_t data[256])
{
    ...
    if (onlp_file_read(data, 256, &size, PORT_FORMAT, PORT_BUS_INDEX(port), "eeprom") != ONLP_STATUS_OK) {
        syslog_ctrl(log_mgmt[port].log_ctrl, SFP_EEPROM_SYSFS_READ_FAIL, "Unable to read eeprom from port(%d)", port);
        return ONLP_STATUS_E_INTERNAL;
    }
    ...
    case ONLP_SFP_CONTROL_TX_DISABLE_CHANNEL:
    {
            ...
            if (present == 1) {
                if (port >= SFP_PORT_MIN && port <= SFP_PORT_MAX) { //SFP
                    ...
                    if(identifier < 0){
                        syslog_ctrl(log_mgmt[port].log_ctrl, SFP_I2C_PREP_FAIL, "Unable to write tx_disable status to port(%d): read identifier from eeprom fail", 
                                port);
                        rv = ONLP_STATUS_E_INTERNAL;
                    }
                        ...
                        if ((status_byte = onlp_sfpi_dev_readb(port, PORT_EEPROM_DEVADDR, QSFP_DD_LOWER_OFFSET_STATUS)) < 0) {
                            syslog_ctrl(log_mgmt[port].log_ctrl, SFP_I2C_PREP_FAIL, "Failed to read Status byte, unable to write tx_disable status to port(%d)", port);
                            rv = ONLP_STATUS_E_INTERNAL;
                            break;
                        }
                        ...
                        if ((rv = onlp_sfpi_dev_writeb(port, PORT_EEPROM_DEVADDR, QSFP_DD_EEPROM_OFFSET_PAGE_SELECT, QSFP_DD_PAGE_ADVERTISING)) < 0) {
                            syslog_ctrl(log_mgmt[port].log_ctrl, SFP_I2C_PREP_FAIL, "Unable to write tx_disable status to port(%d): write page to eeprom fail",
                                    port);
                            goto restore;
                        }
                        if ((eeprom_control = onlp_sfpi_dev_readb(port, PORT_EEPROM_DEVADDR, QSFP_DD_EEPROM_P01H_OFFSET_CONTROL_1)) < 0) {
                            syslog_ctrl(log_mgmt[port].log_ctrl, SFP_I2C_PREP_FAIL, "Unable to write tx_disable status to port(%d): read control from eeprom fail",
                                    port);
                            rv = eeprom_control;
                            goto restore;
                        }
                        if (eeprom_control & QSFP_DD_P01H_TX_DISABLE_SUPPORT) {
                            if ((rv = onlp_sfpi_dev_writeb(port, PORT_EEPROM_DEVADDR, QSFP_DD_EEPROM_OFFSET_BANK_SELECT, 0)) < 0) {
                                syslog_ctrl(log_mgmt[port].log_ctrl, SFP_I2C_PREP_FAIL, "Unable to write tx_disable status to port(%d): write bank to eeprom fail",
                                        port);
                                goto restore;
                            }
                            ...
                            if ((rv = onlp_sfpi_dev_writeb(port, PORT_EEPROM_DEVADDR, QSFP_DD_EEPROM_P10H_OFFSET_OUTPUT_DISABLE_TX, (value & 0xff))) < 0) {
                                syslog_ctrl(log_mgmt[port].log_ctrl, SFP_I2C_WRITE_TARGET_FAIL, "Unable to write tx_disable status to port(%d): write TX disable to eeprom fail",
                                        port);
                                goto restore;
                            }
                        } else {
                    ...
                    restore:
                        if (onlp_sfpi_dev_writeb(port, PORT_EEPROM_DEVADDR, QSFP_DD_EEPROM_OFFSET_PAGE_SELECT, QSFP_DD_PAGE_ADMIN_INFO) < 0) {
                            syslog_ctrl(log_mgmt[port].log_ctrl, SFP_I2C_PREP_FAIL, "Failed to restore Page Select to Admin Info on port(%d)!", port);
                        }
    ...
    case ONLP_SFP_CONTROL_LP_MODE:
        {
            ...
            if (present == 1) {
                identifier = onlp_sfpi_dev_readb(port, PORT_EEPROM_DEVADDR, QSFP_EEPROM_OFFSET_IDENTIFIER);
                if(identifier < 0){
                    syslog_ctrl(log_mgmt[port].log_ctrl, SFP_I2C_PREP_FAIL, "Unable to write LP mode status to port(%d): read identifier from eeprom fail", 
                            port);
                    rv = ONLP_STATUS_E_INTERNAL;
                }
                else if (identifier == QSFP_DD_IDENTIFIER) { /*QSFP DD*/
                    ...
                    if(lpmode_value < 0){
                        syslog_ctrl(log_mgmt[port].log_ctrl, SFP_I2C_READ_TARGET_FAIL, "Unable to write LP mode status to port(%d): read LP mode value from eeprom fail", 
                                port);
                        rv = ONLP_STATUS_E_INTERNAL;
                    }
                    else{
                        ...
                        if(onlp_sfpi_dev_writeb(port, PORT_EEPROM_DEVADDR, QSFP_DD_EEPROM_OFFSET_LOWPWR_REQUEST_SW, lpmode_value) < 0){
                            syslog_ctrl(log_mgmt[port].log_ctrl, SFP_I2C_WRITE_TARGET_FAIL, "Unable to write LP mode status to port(%d): write LP mode value to eeprom fail", 
                                    port);
                            rv = ONLP_STATUS_E_INTERNAL;
                        }
                        else {
                    ...
                    if(lpmode_value < 0){
                        syslog_ctrl(log_mgmt[port].log_ctrl, SFP_I2C_READ_TARGET_FAIL, "Unable to write LP mode status to port(%d): write LP mode value to eeprom fail", 
                                port);
                        rv = ONLP_STATUS_E_INTERNAL;
                    }
                    else{
    ...
    case ONLP_SFP_CONTROL_TX_DISABLE_CHANNEL:
    {
        ...
        if(present == 1) {
            if (port >= SFP_PORT_MIN && port <= SFP_PORT_MAX) {
                ...
                identifier = onlp_sfpi_dev_readb(port, PORT_EEPROM_DEVADDR, QSFP_EEPROM_OFFSET_IDENTIFIER);
                if(identifier < 0){
                    syslog_ctrl(log_mgmt[port].log_ctrl, SFP_I2C_PREP_FAIL, "Unable to read tx_disabled status from port(%d): read identifier from eeprom fail", 
                            port);
                    rv = ONLP_STATUS_E_INTERNAL;
                }
                else if (identifier == QSFP_DD_IDENTIFIER) {/* QSFP DD */
                    /* Flat-memory CMIS modules do not implement page 01h/10h */
                    if ((status_byte = onlp_sfpi_dev_readb(port, PORT_EEPROM_DEVADDR, QSFP_DD_LOWER_OFFSET_STATUS)) < 0) {
                        syslog_ctrl(log_mgmt[port].log_ctrl, SFP_I2C_PREP_FAIL, "Failed to read Status byte, unable to read tx_disable status from port(%d)", port);
                        rv = ONLP_STATUS_E_INTERNAL;
                        break;
                    }
                    if (status_byte & QSFP_DD_FLAT_MEM) {
                    ...
                    if ((rv = onlp_sfpi_dev_writeb(port, PORT_EEPROM_DEVADDR, QSFP_DD_EEPROM_OFFSET_PAGE_SELECT, QSFP_DD_PAGE_ADVERTISING)) < 0) {
                        syslog_ctrl(log_mgmt[port].log_ctrl, SFP_I2C_PREP_FAIL, "Failed to switch to Advertising Page on port(%d)", port);
                        goto restore;
                    }
                    if ((support_ctrls = onlp_sfpi_dev_readb(port, PORT_EEPROM_DEVADDR, QSFP_DD_EEPROM_P01H_OFFSET_CONTROL_1)) < 0) {
                        syslog_ctrl(log_mgmt[port].log_ctrl, SFP_I2C_PREP_FAIL, "Failed to read Support Control on port(%d)", port);
                        rv = support_ctrls;
                        goto restore;
                    }
                    ...
                    if ((rv = onlp_sfpi_dev_writeb(port, PORT_EEPROM_DEVADDR, QSFP_DD_EEPROM_OFFSET_BANK_SELECT, 0)) < 0) {
                        syslog_ctrl(log_mgmt[port].log_ctrl, SFP_I2C_PREP_FAIL, "Unable to read tx_disable status from port(%d): write bank to eeprom fail",
                                port);
                        goto restore;
                    }
                    if ((rv = onlp_sfpi_dev_writeb(port, PORT_EEPROM_DEVADDR, QSFP_DD_EEPROM_OFFSET_PAGE_SELECT, QSFP_DD_PAGE_LANE_CTRL)) < 0) {
                        syslog_ctrl(log_mgmt[port].log_ctrl, SFP_I2C_PREP_FAIL, "Failed to switch to Lane Control Page (Page 0x%02x) on port(%d)",
                    ...
                    if ((tx_dis = onlp_sfpi_dev_readb(port, PORT_EEPROM_DEVADDR, QSFP_DD_EEPROM_P10H_OFFSET_OUTPUT_DISABLE_TX)) < 0) {
                        syslog_ctrl(log_mgmt[port].log_ctrl, SFP_I2C_READ_TARGET_FAIL, "Unable to read tx_disable status from port(%d): read TX disable from eeprom fail",
                                port);
                        rv = tx_dis;
                        goto restore;
                    }
    ...
    case ONLP_SFP_CONTROL_LP_MODE:
        {
            ...
            if (present == 1) {
                identifier = onlp_sfpi_dev_readb(port, PORT_EEPROM_DEVADDR, QSFP_EEPROM_OFFSET_IDENTIFIER);
                if(identifier < 0){
                    syslog_ctrl(log_mgmt[port].log_ctrl, SFP_I2C_PREP_FAIL, "Unable to read LP mode status from port(%d): read identifier from eeprom fail", 
                            port);
                    rv = ONLP_STATUS_E_INTERNAL;
                }
                else if (identifier == QSFP_DD_IDENTIFIER) { /* QSFP DD */
                    ...
                    lpmode_value = onlp_sfpi_dev_readb(port, PORT_EEPROM_DEVADDR, QSFP_DD_EEPROM_OFFSET_LOWPWR_REQUEST_SW);
                    if(lpmode_value < 0){
                        syslog_ctrl(log_mgmt[port].log_ctrl, SFP_I2C_READ_TARGET_FAIL, "Unable to read LP mode status from port(%d): set LP mode value to eeprom fail", 
                                port);
                        rv = ONLP_STATUS_E_INTERNAL;
                    }
                    else {

...
onlp_sfpi_presence_bitmap_get(onlp_sfp_bitmap_t* dst)
{
    ...
    if(fp == NULL) {
        syslog(LOG_ERR, "Unable to open the module_present_all device file from (%s).", MODULE_PRESENT_ALL_ATTR);
        return ONLP_STATUS_E_INTERNAL;
    }
    ...
    if(count != 7) {
        ...
        syslog(LOG_ERR, "Unable to read all fields the module_present_all device file from(%s).", MODULE_PRESENT_ALL_ATTR);
        return ONLP_STATUS_E_INTERNAL;
    }
...
onlp_sfpi_rx_los_bitmap_get(onlp_sfp_bitmap_t* dst)
{
    ...
    if(fp == NULL) {
        syslog(LOG_ERR, "Unable to open the module_rxlos_all device file from (%s).", MODULE_RXLOS_ALL_ATTR);
        return ONLP_STATUS_E_INTERNAL;
    }
    ...
    if(count != 6) {
        /* Likely a CPLD read timeout. */
        syslog(LOG_ERR, "Unable to read all fields the module_rxlos_all device file from(%s).", MODULE_RXLOS_ALL_ATTR);
        return ONLP_STATUS_E_INTERNAL;
    }    
...
static int read_byte(int offset, const char *fmt, ...)
{
    FILE *f;
    uint8_t value = 0;
    char path[100];
...
static int write_byte(int offset, uint8_t value, const char *fmt, ...)
{
    FILE *f;
    char path[100];
    va_list args;
...
int onlp_sfpi_eeprom_readb(int port, uint8_t addr)
{
    /* Validate port */
    if (port < 0) {
    ...
    return read_byte(addr, PORT_EEPROM_FORMAT, (port+1));
}

int onlp_sfpi_eeprom_writeb(int port, uint8_t addr, uint8_t value)
{
    /* Validate port */
    if (port < 0) {
        return ONLP_STATUS_E_PARAM;
    ...
    return write_byte(addr, value, PORT_EEPROM_FORMAT, (port+1));
}

...
static inline int fpga_read(void __iomem *addr, u32 spi_mask, u8 *val)
{
	int ret;
			   ...
			   char *buf)
{
	...
	case MODULE_PRESENT_1 ... MODULE_RX_LOS_66:
		...
		if ((reg & 0xF000) == CPLD1_PCIE_START_OFFSET) {
			ret = fpga_read(fpga_ctl->pci_fpga_dev.data_base_addr1 +
					reg, SPI_BUSY_MASK_CPLD1, &reg_val);
		} else if ((reg & 0xF000) == CPLD2_PCIE_START_OFFSET) {
			ret = fpga_read(fpga_ctl->pci_fpga_dev.data_base_addr2 +
					reg, SPI_BUSY_MASK_CPLD2, &reg_val);
		} else {
			ret = -EINVAL;
		}
		UNLOCK(&cpld_access_lock);

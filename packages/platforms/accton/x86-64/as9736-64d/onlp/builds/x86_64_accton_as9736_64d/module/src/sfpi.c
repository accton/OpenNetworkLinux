			...
			if (port >= SFP_PORT_MIN && port <= SFP_PORT_MAX) { //SFP
				if (onlp_file_write_int(value, MODULE_TXDISABLE_FORMAT,
					...
					syslog_ctrl(&log_mgmt[port], TX_DIS_UNABLE_TO_SET_STATUS,
						    "Unable to write tx_disable status to port(%d)",
						    port);
				...
				if (identifier < 0) {
					...
					if ((rv = onlp_sfpi_eeprom_writeb(port, QSFP_DD_EEPROM_OFFSET_PAGE_SELECT, QSFP_DD_PAGE_ADVERTISING)) < 0) {
						syslog_ctrl(&log_mgmt[port], TX_DIS_UNABLE_TO_SET_EEPROM_PAGE,
							    "Unable to write tx_disable status to port(%d): write page to eeprom fail",
							    port);
						goto restore;
					}
					if ((eeprom_control = onlp_sfpi_eeprom_readb(port, QSFP_DD_EEPROM_P01H_OFFSET_CONTROL_1)) < 0) {
						syslog_ctrl(&log_mgmt[port], TX_DIS_UNABLE_TO_GET_CONTROL,
							    "Unable to write tx_disable status to port(%d): read control from eeprom fail",
							    port);
						rv = eeprom_control;
						goto restore;
					}
					if (eeprom_control & QSFP_DD_P01H_TX_DISABLE_SUPPORT) {
						if ((rv = onlp_sfpi_eeprom_writeb(port, QSFP_DD_EEPROM_OFFSET_BANK_SELECT, 0)) < 0) {
							syslog_ctrl(&log_mgmt[port], TX_DIS_UNABLE_TO_SET_BANK,
								    "Unable to write tx_disable status to port(%d): write bank to eeprom fail",
								    port);
							goto restore;
						}
						if ((rv = onlp_sfpi_eeprom_writeb(port, QSFP_DD_EEPROM_OFFSET_PAGE_SELECT, QSFP_DD_PAGE_LANE_CTRL)) < 0) {
							syslog_ctrl(&log_mgmt[port], TX_DIS_UNABLE_TO_SET_EEPROM_PAGE,
								    "Unable to write tx_disable status to port(%d): write page to eeprom fail",
								    port);
							goto restore;
						}
						if ((rv = onlp_sfpi_eeprom_writeb(port, QSFP_DD_EEPROM_P10H_OFFSET_OUTPUT_DISABLE_TX, (value & 0xff))) < 0) {
							syslog_ctrl(&log_mgmt[port], TX_DIS_UNABLE_TO_SET_STATUS,
	...
	case ONLP_SFP_CONTROL_RESET:
		...
		if (onlp_file_write_int(value, MODULE_RESET_FORMAT,
			...
			syslog_ctrl(&log_mgmt[port], RESET_UNABLE_TO_SET_STATUS,
				    "Unable to write reset status to port(%d)",
				    port);
			rv = ONLP_STATUS_E_INTERNAL;
		}
		else {
	...
	case ONLP_SFP_CONTROL_LP_MODE:
		...
		if (onlp_file_write_int(value, MODULE_LPMODE_FORMAT,
			...
			syslog_ctrl(&log_mgmt[port], LP_MODE_UNABLE_TO_SET_STATUS,
				    "Unable to write LP mode status to port(%d)",
				    port);
			rv = ONLP_STATUS_E_INTERNAL;
		}
		else {
		...
		if (present == 1) {
			if (port >= SFP_PORT_MIN && port <= SFP_PORT_MAX) {
				...
				if (identifier < 0) {
					...
					if ((support_ctrls = onlp_sfpi_eeprom_readb(port, QSFP_DD_EEPROM_P01H_OFFSET_CONTROL_1)) < 0) {
						syslog_ctrl(&log_mgmt[port], TX_DIS_UNABLE_TO_GET_CONTROL,
					...
					if ((rv = onlp_sfpi_eeprom_writeb(port, QSFP_DD_EEPROM_OFFSET_BANK_SELECT, 0)) < 0) {
						syslog_ctrl(&log_mgmt[port], TX_DIS_UNABLE_TO_SET_BANK,
							    "Unable to read tx_disable status from port(%d): write bank to eeprom fail",
							    port);
						goto restore;
					}
					if ((rv = onlp_sfpi_eeprom_writeb(port, QSFP_DD_EEPROM_OFFSET_PAGE_SELECT, QSFP_DD_PAGE_LANE_CTRL)) < 0) {
					...
					if ((tx_dis = onlp_sfpi_eeprom_readb(port, QSFP_DD_EEPROM_P10H_OFFSET_OUTPUT_DISABLE_TX)) < 0) {
						syslog_ctrl(&log_mgmt[port], TX_DIS_UNABLE_TO_GET_STATUS,
	...
	case ONLP_SFP_CONTROL_RESET:
		...
		if (onlp_file_read_int(value, MODULE_RESET_FORMAT,
			...
			syslog_ctrl(&log_mgmt[port], RESET_UNABLE_TO_GET_STATUS,
				    "Unable to read reset status from port(%d)",
				    port);
			rv = ONLP_STATUS_E_INTERNAL;
		}
		else {
	...
	case ONLP_SFP_CONTROL_LP_MODE:
		...
		if (onlp_file_read_int(value, MODULE_LPMODE_FORMAT,
			...
			syslog_ctrl(&log_mgmt[port], LP_MODE_UNABLE_TO_GET_STATUS,
				    "Unable to read LP mode status from port(%d)",
				    port);
			rv = ONLP_STATUS_E_INTERNAL;
		}
		else {
